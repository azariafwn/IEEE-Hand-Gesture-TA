...
Beyond static testing, the ultimate viability of a smart home control system relies on its real-time responsiveness and sequential logic accuracy. The proposed system was subjected to a comprehensive live evaluation on the Raspberry Pi 5 to assess both its computational speed and the robustness of the State Machine in handling sequential commands (i.e., combining a Selection Gesture with an Action Gesture).

To quantify the real-time processing capabilities, the overall system speed (Frames Per Second, FPS) and the isolated edge inference latency of the TFLite model were recorded. The statistical summary is presented in Table \ref{tab:edge_speed}.
...
The system demonstrated exceptional operational stability, maintaining a macro-average F1-Score of 0.99 across all combined sequences. This proves that the integration of the quantized LSTM model with the strict temporal State Machine logic effectively neutralizes false positive triggers, resulting in a highly reliable, always-on smart home control interface.
